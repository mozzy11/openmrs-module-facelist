	...
	<div class="panel-group">
		...
		<div class="panel panel-default">
			...
			<form method="POST" modelAttribute="fileUpload" enctype="multipart/form-data" id="upload">
				<div>
					<c:out value=" PATIENT NAME :" /><select class="btn btn-success btn-large" id="selected"
						name="Patient_ID" tyoe="number">

						<c:forEach items="${patients}" var="item">
							<option value="${item.patientId}" ${item.patientId==selectedRole ? 'selected' : '' }>
								<c:out value="${item.personName}" />
							</option>
						</c:forEach>
					</select>
...
<div class="container">
	...
	<a href="Jquerytable.form"><button type="button" class="btn btn-info">
			<c:out value="VIEW FACE LIST" /></button> </a>
